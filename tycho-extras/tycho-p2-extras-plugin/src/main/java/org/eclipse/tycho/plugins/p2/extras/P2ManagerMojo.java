...
public class P2ManagerMojo extends AbstractMojo {
	...
	/**
	 * The repositories where the P2 Manager application should be sourced from
	 */
	@Parameter
	...
	public void execute() throws MojoExecutionException, MojoFailureException {
		...
		List<MavenRepositoryLocation> locations = getRepositoryLocations();
		TargetPlatform targetPlatform = applicationFactory.createTargetPlatform(getRepositoryLocations());
	...
	private List<MavenRepositoryLocation> getRepositoryLocations() {
		if (repositories == null || repositories.isEmpty()) {
			MavenRepositoryLocation eclipseLocation = new MavenRepositoryLocation(null,
					URI.create(TychoConstants.ECLIPSE_LATEST));
			MavenRepositoryLocation justjToolsLocation = new MavenRepositoryLocation(null,
					URI.create(JUST_TOOLS_NIGHTLY));
			return List.of(eclipseLocation, justjToolsLocation);
		}
		return repositories.stream().map(repo -> new MavenRepositoryLocation(repo.getId(), repo.getLocation()))
				.toList();
	}
}
