...
public class RepoRefLocationP2RepositoryIntegrationTest extends AbstractTychoIntegrationTest {
    ...
    public void testRefLocation() throws Exception {
        ...
        artifactsDocument.getChild("repository").getChild("repository-references").getChildren("repository-reference")
                .forEach(element -> {
                    String location = element.getAttributeValue("location");
                    if (expected.containsKey(location)
                            && expected.get(location).equals(element.getAttributeValue("enabled"))) {
