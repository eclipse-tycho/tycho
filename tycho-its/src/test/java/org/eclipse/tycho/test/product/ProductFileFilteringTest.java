...
public class ProductFileFilteringTest extends AbstractTychoIntegrationTest {
    ...
    public void test() throws Exception {
        Verifier verifier = getVerifier("/product.sourceFile.filtering", false);
        ...
        InputStream is = new BufferedInputStream(new FileInputStream(new File(basedir,
                "target/products/psf.product/linux/gtk/x86/configuration/config.ini")));
        try {
            p.load(is);
        } finally {
            IOUtil.close(is);
        ...
        assertEquals("1.0.0.123abc", p.getProperty("eclipse.buildId"));
    }
}
