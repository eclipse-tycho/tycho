...
public class ArtifactPublishingAndAssemblyTest extends AbstractTychoIntegrationTest {
    ...
    /**
     * The expanded version of projects with Maven version "1.0.0-SNAPSHOT" which didn't force a
     * different qualifier than the parent POM.
     */
    ...
    public static void executeBuild() throws Exception {
        verifier = new ArtifactPublishingAndAssemblyTest().getVerifier("p2Repository.reactor", false);
    ...
    @Test
    public void testPublishedCategoryIU() throws Exception {
    ...
    @Test
    public void testPublishedFeatureIU() throws Exception {
        String featureIuId = "prr.example.feature" + ".feature.group";
        IU featureIU = p2Repository.getIU(featureIuId, FEATURE_VERSION);

        assertThat(featureIU.getProperties(), hasItem("org.eclipse.equinox.p2.type.group=true"));
        assertThat(featureIU.getInclusions(),
                hasItem(withIdAndVersion("prr.example.included.feature.feature.group", DEFAULT_VERSION)));
        assertThat(featureIU.getInclusions(), hasItem(withIdAndVersion("prr.example.bundle", BUNDLE_VERSION)));
        assertThat(featureIU.getInclusions(),
                hasItem(withIdAndVersion("org.eclipse.core.contenttype", "3.4.1.R35x_v20090826-0451"))); // a bundle from the external target platform
    ...
    // TODO 373817 test that inclusions in products have the right qualifier
