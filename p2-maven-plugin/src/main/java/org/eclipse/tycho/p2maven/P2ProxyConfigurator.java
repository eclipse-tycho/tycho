 ...
 *    SAP AG  - API and implementation that was moved from ProxyServiceFacadeImpl here
 *    Christoph Läubrich - merge hybrid-code into this one
 *******************************************************************************/
...
public class P2ProxyConfigurator implements EquinoxLifecycleListener {
	...
	public void afterFrameworkStarted(EmbeddedEquinox framework) {
		...
		LogReaderService logReaderService = framework.getServiceFactory().getService(LogReaderService.class);
		logReaderService.addLogListener(new LogListener() {

			@Override
			public void logged(LogEntry entry) {
				System.out.println(entry.getMessage() + " // " + entry.getException());
	...
	private void configureProxy(IProxyService proxyService, String protocol, String host, int port, String user,
		...
		// have to register authenticator manually as this is provided as extension
		// point in
	...
	private void registerAuthenticator(final String user, final String password) {
		// FIXME is this really still required?
