 ...
 * </p>
 */
@Mojo(name = "modify-metadata", threadSafe = true)
public class ModifyRepositoryMetadataMojo extends AbstractMojo {
    ...
    /**
     * The kind of modified repository: Either {@code artifact}, {@code metadata} or
     * {@code both}.<br>
     * If {@code artifact} is specified, only the {@code artifact.xml/jar/xml.xz} files are
     * modified, if {@code metadata} only {@code content.xml/jar/xml.xz}. By default {@code both}
     * are modified.
     */
    @Parameter(property = "p2.repository.kind")
    private P2RepositoryKind repositoryKind = P2RepositoryKind.both;
    ...
    public void execute() throws MojoExecutionException, MojoFailureException {
        ...
        if (repositoryKind != P2RepositoryKind.both) {
            type = (repositoryKind.isArtifact() ? "artifact " : "metadata") + " repository";
        }
        String location = descriptor.sourceRepository().getUrl();
        Path output = descriptor.outputLocation();
        if (output != null) {
            logger.info("  Write modified " + type + " file(s) to: " + output);
        } else {
            logger.info("  Modify " + type + " in-place at: " + location);
        }
        if (repositoryName != null) {
            logger.info("  Set name to: " + repositoryName);
        }
        if (!propertiesToRemove.isEmpty()) {
            logger.info("  Remove properties:" + String.join(", ", propertiesToRemove));
        }
        ...
        } catch (ProvisionException | IOException | URISyntaxException e) {
            throw new MojoExecutionException("Failed to modify repository " + location, e);
        }
    }
