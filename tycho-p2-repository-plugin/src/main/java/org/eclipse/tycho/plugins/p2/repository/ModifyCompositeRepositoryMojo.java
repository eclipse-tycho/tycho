...
/**
 * Mojo that provides similar functionality as the <a href=
 * "https://help.eclipse.org/latest/topic/org.eclipse.platform.doc.isv/guide/p2_repositorytasks.htm">{@code p2.composite.repository}
 * ANT task</a>.
 */
// TODO: more details and check properties
...
public class ModifyCompositeRepositoryMojo extends AbstractMojo {
    ...
    private boolean validate; //TODO: imply false if no source is given?
    ...
    public void execute() throws MojoExecutionException, MojoFailureException {
        ...
        try {
            ...
            if (validate && !childrenToAdd.isEmpty()) {
    ...
    private void validateChildRepository(URI child, ModifiedRepositoryDescriptor baseRepository)
        ...
        if (baseRepository.isArtifact()) {
            artifacts = manager.countArtifacts(childLocation, repositoryLocation.getId());
        }
        if (baseRepository.isMetadata()) {
            units = manager.countMetadataUnits(childLocation, repositoryLocation.getId());
        }
        log.info("  Found " + (artifacts != null ? artifacts + " artifacts" : "")
                + (artifacts != null && units != null ? " and " : "") + (units != null ? units + " units" : "") + ".");
    }
