...
public final class TargetDefinitionResolver {
    ...
    private static final String SOURCE_IU_ID = "org.eclipse.tycho.target.source.bundles";
            ...
            if (includeSource && !units.isEmpty()) {
                ...
                for (IInstallableUnit unit : units) {
                    if (unit.satisfies(bundleRequirement)) {
                        requirements.add(MetadataFactory.createRequirement("osgi.bundle", unit.getId() + ".source",
                                new VersionRange(unit.getVersion(), true, unit.getVersion(), true), null, true, false,
                                true));
                    }
                }
                ...
                Version sourceIUVersion = Version.createOSGi(1, 0, 0);
                sourceDescription.setVersion(sourceIUVersion);
                sourceDescription.addRequirements(requirements);
                IProvidedCapability capability = MetadataFactory.createProvidedCapability(
