...
public class FileRepositoryRolver implements Resolvable {
    ...
    public FileRepositoryRolver(IProvisioningAgent agent, File location) {
    ...
    public IMetadataRepository getIMetadataRepository() {
    ...
    public Collection<IInstallableUnit> resolve(IProgressMonitor monitor) throws ResolverException {
        ...
        return Collections.unmodifiableCollection(repositoryContent.values());
    }
    ...
    public Collection<IInstallableUnit> getUnits(IArtifactKey key) {
        List<IInstallableUnit> list = new ArrayList<>();
        for (Entry<IArtifactDescriptor, IInstallableUnit> entry : repositoryContent.entrySet()) {
    ...
    public static void readFeatures(File path, BiConsumer<IArtifactDescriptor, IInstallableUnit> consumer,
    ...
    public static void readBundles(File path, BiConsumer<IArtifactDescriptor, IInstallableUnit> consumer,
            IProgressMonitor monitor) throws ResolverException {
        PublisherInfo publisherInfo = new PublisherInfo();
        publisherInfo.setArtifactOptions(IPublisherInfo.A_INDEX);
        if (path.isDirectory()) {
    ...
    private static class FileArtifactRepository extends AbstractArtifactRepository implements IFileArtifactRepository {
        ...
        public IStatus getRawArtifact(IArtifactDescriptor descriptor, OutputStream destination,
            ...
            if (artifactFile == null) {
                return new Status(IStatus.ERROR, FileRepositoryRolver.class.getName(), "Artifact not found");
            }
            try {
                ...
                return new Status(IStatus.ERROR, FileRepositoryRolver.class.getName(), "transfer failed", e);
            }
        ...
        public IStatus getArtifacts(IArtifactRequest[] requests, IProgressMonitor monitor) {
            ...
            MultiStatus multiStatus = new MultiStatus(FileRepositoryRolver.class.getName(), IStatus.INFO,
                    "Request Status");
            boolean ok = true;
