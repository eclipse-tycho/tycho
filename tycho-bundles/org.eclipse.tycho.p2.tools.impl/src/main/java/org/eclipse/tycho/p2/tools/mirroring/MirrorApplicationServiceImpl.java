...
public class MirrorApplicationServiceImpl implements MirrorApplicationService {
    ...
    @Override
    public void addMavenMappingRules(File repository, URI[] mavenRepositories, boolean deleteRepositoryArtifactCopies)
            throws FacadeException {
        ...
        for (IArtifactDescriptor artifact : repo.getDescriptors()) {
            ...
            if (gav != null && !gav.getVersion().endsWith("-SNAPSHOT")) {
                for (URI mavenRepo : mavenRepositories) {
                    ...
                    URI mavenArtifactURI = URI.create(mavenRepo.toString() + '/' + gav.getGroupId().replace('.', '/')
                            + '/' + gav.getArtifactId() + '/' + gav.getVersion() + "/" + gav.getArtifactId() + '-'
                            + gav.getVersion() + mavenClassifier + ".jar");
                    try {
