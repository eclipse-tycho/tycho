...
class ProgressCleaningLogger implements MavenLogger {
    ...
    public void info(String message) {
        if ("".equals(message) || "1 operation remaining.".equals(message)) {
            // filter out
            return;
        }
        ...
        if (startOfByteProgress > 0) {
            if (startOfByteProgress == lastLoggedFile.length() && message.startsWith(lastLoggedFile)) {
                if (!lastLoggedFileFiltered
                    ...
                    /*
                     * p2 always seems to print at least two "Fetching ..." lines for the same file.
                     * Remove the second one, which doesn't really add anything. Later lines may be
                     * interesting because they include the download speed.
                     */
                    // (1,95kB at 0B/s)
