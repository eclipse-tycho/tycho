...
public class EnvironmentSpecificConfigurations {
    ...
    public static EnvironmentSpecificConfigurations parse(EnvironmentSpecificConfiguration globalConfiguration,
            PlexusConfiguration rawConfiguration) throws MojoExecutionException {
        return new EnvironmentSpecificConfigurations(globalConfiguration, parseSpecificConfigurations(rawConfiguration));
    }

    private static List<Entry> parseSpecificConfigurations(PlexusConfiguration rawConfiguration)
            throws MojoExecutionException {
        List<Entry> result = new ArrayList<Entry>();

        for (PlexusConfiguration envSpecificTag : rawConfiguration.getChildren()) {
            result.add(new Entry(envSpecificTag.getName(), EnvironmentSpecificConfiguration.parse(envSpecificTag)));
        }
        return result;
    }
     ...
     * explicit value specified for a less specific environment filter. In case of multiple matching
     * configurations with same number of criteria (os/ws/arch), the first configuration wins. Local
     * configuration is placed before the configuration inherited from a parent POM, so this choice
     * leads to a slight preference for POM-local configuration (see also
     * http://jira.codehaus.org/browse/MNG-5474).
     * 
     * @param parameter
