...
public class DefaultEquinoxInstallationFactoryTest {
    ...
    public void testBundleStartLevelsWithDefaultStartLevel() throws IOException {
        ...
        Assert.assertTrue("osgi string must contain bundle1 and its start level!",
                osgiBundles.contains("reference:file:absolute/path/to/bundle1@6:start"));
        Assert.assertTrue("osgi string must contain bundle2 and the default start level!",
                osgiBundles.contains("reference:file:absolute/path/to/bundle2@7:start"));
    }
    ...
    public void testBundleStartLevelsWithoutDefaultStartLevel() throws IOException {
        ...
        Assert.assertTrue("osgi string must contain bundle1 and its start level!",
                osgiBundles.contains("reference:file:absolute/path/to/bundle1@6:start"));
        Assert.assertTrue("osgi string must contain bundle2!",
                osgiBundles.contains("reference:file:absolute/path/to/bundle2"));
        Assert.assertFalse("osgi string must contain bundle2 witout any start level!",
                osgiBundles.contains("reference:file:absolute/path/to/bundle2@"));
    }
}
