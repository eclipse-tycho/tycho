...
public abstract class AbstractOsgiCompilerMojo extends AbstractCompilerMojo
    ...
    protected final List<String> getCompileSourceExcludePaths() throws MojoExecutionException {
        ...
        for (File files : outputJar.getFilesToExclude()) {
            excludes.add(new File(files.getAbsoluteFile().toURI().normalize()).toString());
        }
        return excludes;
    }

    @Override
    protected final List<File> getCompileSourceExcludeFiles() throws MojoExecutionException {
        ArrayList<File> excludes = new ArrayList<>();
        for (File files : outputJar.getFilesToExclude()) {
            excludes.add(new File(files.getAbsoluteFile().toURI().normalize()));
        }
        return excludes;
    }
