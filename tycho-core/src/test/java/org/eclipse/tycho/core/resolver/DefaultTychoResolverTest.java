...
public class DefaultTychoResolverTest {
    ...
    public void testTychoEnvironmentPropertiesGetSet() {
        ...
        MavenSession session = EasyMock.createMock(MavenSession.class);
        MavenProject project = EasyMock.createMock(MavenProject.class);
        Properties systemProperties = new Properties();
        Properties projectProperties = new Properties();
        EasyMock.expect(session.getSystemProperties()).andReturn(systemProperties).anyTimes();
        EasyMock.expect(project.getProperties()).andReturn(projectProperties).anyTimes();
        EasyMock.replay(session, project);

        defaultTychoResolver.setTychoEnvironmentProperties(session, project);
