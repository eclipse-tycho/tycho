...
public class ForkClient implements StreamConsumer {
    ...
    private ReportEntry createReportEntry(String untokenized) {
        ...
        try {
            ...
            final StackTraceWriter stackTraceWriter = tokens.hasMoreTokens() ? deserializeStackStraceWriter(tokens)
                    : null;

            return group != null ? new CategorizedReportEntry(source, name, group, stackTraceWriter, elapsed)
    ...
    private StackTraceWriter deserializeStackStraceWriter(StringTokenizer tokens) {
        ...
        stackTraceWriter = stackTrace != null ? new DeserializedStacktraceWriter(stackTraceMessage, stackTrace) : null;
    ...
    public void close() {
        /*
         * Iterator iter = testSetReporters.values().iterator(); while( iter.hasNext() ) {
