...
public class TestMojo extends AbstractMojo {
     ...
     * (optionally) the product IU under test. This can be used for integration tests that require a
     * fully p2-provisioned installation. Note that this comes with a performance overhead for
     * executing the provisioning operations otherwise not required.</li>
     * <li>If <code>false</code>, all necessary files to define the installation under test like
     * <tt>config.ini</tt> are generated by tycho. This has the advantage that the installation is
     * minimal (defined by the transitive dependencies of the test bundle plus the test harness) and
     * existing bundle jars can be referenced rather than copied for the installation</li>
     * </ul>
     * </p>
     * 
     * @parameter default-value="false"
    ...
    /**
     * <p>
     * Name of IU under test to be installed using p2 director. Optional and only considered if
     * {@link #provisionInstallation} is <code>true</code>. The IU to be installed as well as its
     * dependencies will be resolved from the project's target platform.
     * 
     * <ol>
     * <li>If not <code>null</code>, the target installation in {@link #work} consisting of
     * {@link #productIU}, test bundle, test harness bundles and respective dependencies will be
     * installed from scratch before running the tests</li>
     * <li>If <code>null</code>, the target installation in {@link #work} is assumed to be already
     * existing, i.e. must be supplied in directory {@link #work} before the test runs by other
     * means. Then only test bundle, test harness and respective dependencies are installed into the
     * existing p2 profile before running the tests. This is intended for running tests on existing
     * installation binaries produced outside of a tycho build.</li>
     * </ol>
     * </p>
     * 
     * @parameter expression="${tycho.testProductIU}"
     * @since 0.19.0
     */
    private String productIU;
    ...
    private EquinoxInstallation createProvisionedInstallation() throws MojoExecutionException {
        try {
            ...
            if (productIU != null) {
                iusToInstall.add(productIU);
                // need to set profile name for from-scratch installations only
                installationBuilder.setProfileName("tychoSurefire");
            }
