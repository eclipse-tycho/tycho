...
public class TestMojo extends AbstractMojo {
    ...
    private EquinoxInstallation createInstallationFromProvisionnedApplication(File targetApplicationPath2,
        ...
        try {
            ...
            String[] segments = this.project.getVersion().split(this.project.getVersion());
            ...
            //res.install(this.projectTypes.get(this.project.getPackaging()).)
            //        final List<Dependency> allDependencies = new ArrayList<Dependency>();
            //        if (this.dependencies != null) {
            //            for (Dependency dep : this.dependencies) {
            //                if (res.getInstallationDescription().getBundle(dep.getArtifactId(), dep.getVersion().))
            //            }
            //        }
            //
