...
public class TestMojoToolchainTests extends TestCase {
    ...
    public void setupWithBree() throws Exception {
        ...
        ExecutionEnvironment env = EasyMock.createMock(ExecutionEnvironment.class);
        EasyMock.expect(env.getProfileName()).andReturn("myId");
        EasyMock.expect(envConf.getFullSpecification()).andReturn(env);
        EasyMock.replay(env, envConf);
