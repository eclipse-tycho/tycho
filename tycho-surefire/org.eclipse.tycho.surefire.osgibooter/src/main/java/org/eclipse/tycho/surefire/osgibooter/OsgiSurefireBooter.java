...
public class OsgiSurefireBooter {
    ...
    public static int run(String[] args) throws Exception {
        ...
        final boolean inForkedVM = true;
        final boolean trimStacktrace = true;
        final boolean useSystemClassloader = false;
        final boolean useManifestOnlyJar = false;
        ...
        if (isDumpMode(args)) {
            ProviderFactory providerFactory = new ProviderFactory(startupConfiguration, providerConfiguration,
                    surefireClassLoader, testClassLoader, null);

            writeTestClassesToFile(providerFactory.createProvider().getSuites(),
                    new File(testProps.getProperty("testlistdumpfile")));
            return 0;//PASCAL Is there a better status?
        }

        if ("always".equals(testProps.get("forkMode"))) {
            RunResult result = surefireStarter.runSuitesInProcessWhenForked();//srunSuitesInProcess();
            return result.getForkedProcessCode();
        }
        if ("none".equals(testProps.get("forkMode"))) {
            RunResult result = surefireStarter.runSuitesInProcess();
            return result.getForkedProcessCode();
        } else
            return 13; //PASCAL is there a better status
    }

    private static void writeTestClassesToFile(Iterator suites, File dumpFile) throws IOException {
        int count = 0;
        Properties allTestsToRun = new Properties();
        while (suites.hasNext()) {
            Class suite = (Class) suites.next();
            allTestsToRun.put("class." + count, suite.getName());
            count++;
        }
        BufferedOutputStream output = new BufferedOutputStream(new FileOutputStream(dumpFile));
        allTestsToRun.store(output, "Generated by OsgiSurefireBooter");
        output.close();

        //PASCAL review stream handling
    }
