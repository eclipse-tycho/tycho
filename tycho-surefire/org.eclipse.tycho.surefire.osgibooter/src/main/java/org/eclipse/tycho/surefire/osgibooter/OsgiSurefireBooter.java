...
public class OsgiSurefireBooter {
    ...
    private static ClassLoader getBundleClassLoader(String symbolicName) throws BundleException {
        ...
        try {
            ...
            if (ex.getType() == BundleException.RESOLVE_ERROR && Activator.INSTANCE != null
                    && Activator.INSTANCE.canUsePlatformAdmin()) {
                Set<ResolverError> errors = Activator.INSTANCE.getResolutionErrors(bundle);
                if (errors.size() > 0) {
